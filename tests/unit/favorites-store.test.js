...
describe('FavoritesStore', () => {
  it('hydrates from warm cache and keeps older cached pages until fresh paging catches up', async () => {
    ...
    expect(snapshot.nextCursor).toBe('page-36');
  });

  it('prefetches enough items to fill partially loaded pages', async () => {
    ...
    await store.hydrate();
    await store.prefetchToCompletePages(store.getSnapshot().requestId);
  ...
  it('loads more pages when navigating beyond loaded pages', async () => {
