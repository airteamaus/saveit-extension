...
describe('toolbar popup', () => {
  ...
  it('renders project buttons from the background response', async () => {
    const sendMessage = vi.fn(async (message) => {
      if (message.action === 'getToolbarProjects') {
        return {
          success: true,
          projects: [
            { id: 'project-1', name: 'SaveIt product' },
    ...
    expect(projectButtons.map((button) => button.textContent)).toEqual(['SaveIt product', 'AI radar']);
  });
  ...
  it('sends the selected project when a project button is clicked', async () => {
    ...
    const sendMessage = vi.fn(async (message) => {
      if (message.action === 'getToolbarProjects') {
        return {
          success: true,
          projects: [{ id: 'project-1', name: 'SaveIt product' }]
        };
      }
