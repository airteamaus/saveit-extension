...
- **Feed row**: the idle index row in org mode — same table-of-contents anatomy as the index row. The owner's own rows carry the full hover-revealed management set (edit · pin · privacy · projects · delete) over the date slot, plus an inline edit form; org-mates' rows have no action slot, so their date never yields. An always-visible vote control (chevron + mono points) leads the meta line; `aria-pressed` marks the voted state; the voter's own row disables the chevron. A mono scope tag ("Only you" / "Shared") is always visible; attribution ("saved by X") rides the same meta line for org-mates only. The header gains a mono scope kicker ("Everyone at acme.com" / "Everyone using Gmail — public" / "Your saves only"), and public scopes get a one-time hairline disclosure above the index.
